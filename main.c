...
int main() {
    ...
    // curl_easy_setopt(curl, CURLOPT_URL, "https://stackoverflow.com/");//Recuperation des informations au niv de l'URL
	...
	if (fp != NULL) // On verifie que le fichier existe et est ouvert...
    {
	    ...
	    while (c != EOF) // tant qu'on est pas à la fin du document...
	    {
	    	...
	    	if(occu == 4)
	    	{
				...
				for(int i = 0; i != occu -1; i++) // Affichage de la balise et de son contenu
	    		{
	    			printf("%c", str[i]);
	    		}
